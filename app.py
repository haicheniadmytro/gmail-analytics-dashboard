...
# --- Функція декодування заголовків листів ---
...
def fetch_emails_imap(email_user, app_password, max_results=200):
    try:
        # Підключаємося до IMAP-сервера Gmail
        mail = imaplib.IMAP4_SSL("imap.gmail.com")
        # Очищаємо пароль від пробілів, якщо вони є
        clean_password = app_password.replace(" ", "")
        mail.login(email_user, clean_password)

        # Обираємо папку Вхідні (INBOX)
        mail.select("INBOX")
        ...
        # Беремо останні N листів
        email_ids_to_fetch = email_ids[-max_results:]
        email_ids_to_fetch.reverse()  # Нові листи спочатку
        ...
        for idx, e_id in enumerate(email_ids_to_fetch):
            status_text.text(f"Завантаження листа {idx + 1} з {len(email_ids_to_fetch)}...")

            # Отримуємо тільки заголовки для прискорення завантаження
            _, msg_data = mail.fetch(e_id, "(BODY.PEEK[HEADER.FIELDS (DATE FROM SUBJECT)])")
        ...
        progress_bar.empty()
        status_text.empty()
        st.success(f"✅ Завантажено {len(email_data)} листів із {total_emails} наявних!")

        return pd.DataFrame(email_data)

    except imaplib.IMAP4.error as e:
        st.error("❌ Помилка авторизації! Перевірте правильність Email та 16-значного Пароля додатка.")
        st.error(f"Деталі: {e}")
        return None
...
def analyze_emails(df):
    ...
    df["sender_domain"] = df["clean_from"].apply(lambda x: x.split("@")[-1] if "@" in x else "")
...
st.markdown("Простий аналіз пошти через IMAP (без складного OAuth)")

with st.sidebar:
    st.header("🔑 Авторизація IMAP")
    user_email = st.text_input("Ваш Gmail / корпоративний email", placeholder="name@domain.com")
    app_password = st.text_input("Пароль додатка (16 символів)", type="password", placeholder="abcd efgh ijkl mnop")
...
if btn_fetch:
    if not user_email or not app_password:
        ...
        with st.spinner("Підключення до пошти..."):
            raw_df = fetch_emails_imap(user_email, app_password, max_results=max_emails)
            if raw_df is not None and not raw_df.empty:
                analyzed_df = analyze_emails(raw_df)
                st.session_state["data"] = analyzed_df

# --- Відображення результатів аналітики ---
if "data" in st.session_state and st.session_state["data"] is not None:
    ...
    tab1, tab2, tab3 = st.tabs(["📈 Часові тренди", "👥 Відправники та домени", "📊 Детальна таблиця"])
    ...
    with tab1:
        ...
        if not df["date_only"].isna().all():
            ...
            st.altair_chart(
                alt.Chart(pd.DataFrame({"date": daily_counts.index, "count": daily_counts.values}))
                .mark_bar()
                .encode(x=alt.X("date:T", title="Дата"), y=alt.Y("count:Q", title="Кількість листів"))
        ...
        if not df["hour"].isna().all():
            ...
            st.altair_chart(
                alt.Chart(pd.DataFrame({"hour": hour_counts.index, "count": hour_counts.values}))
                .mark_bar(color="steelblue")
                .encode(x=alt.X("hour:O", title="Година (0-23)"), y=alt.Y("count:Q", title="Кількість листів"))
    ...
    with tab2:
        ...
        with col_senders:
            ...
            st.dataframe(
                pd.DataFrame({"Email відправника": top_senders.index, "Кількість": top_senders.values}),
                use_container_width=True,
            )
    ...
    with tab3:
        ...
        st.dataframe(df[["parsed_date", "clean_from", "subject", "is_reply"]], use_container_width=True)

        csv = df.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="📥 Завантажити CSV", data=csv, file_name="gmail_imap_analytics.csv", mime="text/csv"
        )
else:
    st.info("👈 Введіть Email і Пароль додатка ліворуч у панелі та натисніть 'Завантажити пошту'.")
